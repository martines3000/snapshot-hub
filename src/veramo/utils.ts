        ...
        if (vp.verifiableCredential) {
          const unresolved: Array<Promise<boolean>> =
            vp.verifiableCredential?.map(async (_vc): Promise<boolean> => {
              ...
              if (
                vc.credentialSubject.id?.split(':')[3].toLowerCase() !=
                address.toLowerCase()
              ) {
              ...
              // 6. verify VP holder
              if (
                vp.holder.toLowerCase() != vc.credentialSubject.id.toLowerCase()
              ) {
        ...
        if (vp.verifiableCredential) {
          const unresolved: Array<Promise<boolean>> =
            vp.verifiableCredential?.map(async (_vcJwt): Promise<boolean> => {
              ...
              // 5. verify issuer
              if (
                vc.iss.toUpperCase() !== process.env.VC_ISSUER?.toUpperCase()
              ) {
                console.log('failed to verify issuer');
                return false;
              }
